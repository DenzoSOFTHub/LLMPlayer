...
> **⚠️ Snapshot from 2026-03-07.** The per-section bandwidth breakdown below is still accurate for the FP32-kernel path, but the headline numbers have moved substantially since then. The dp4a wiring sweep across v1.10.1 → v1.11.0-dev → v1.13.0 brought Llama-3.2-1B Q4_K_M from 47 to roughly 63 tok/s on the same RTX 4050, Phi-3-mini IQ4_NL from 8.4 to 11.9 (+42 %), Nemotron-3-Nano-4B from 10.4 to 20.0 (+92 %), Llama-3.2-3B Q3_K_L from 12.5 to 14.5 (+15.5 %, v1.13.0 Q3_K dp4a kernel), and the cumulative Gemma-3-1B figure to about 43.6 tok/s (~+36 % across the v1.13.0 changes alone). The v1.12.0 CPU SIMD rewrite reshaped the no-GPU column equally hard: B2I/I2F lane-parallel rewrites of `SimdQ6_KFloatTensor` / `SimdQ8_0FloatTensor` / `SimdQ5_KFloatTensor` / `SimdQ5_0FloatTensor` / `SimdQ3_KFloatTensor` doubled or tripled CPU tok/s on most small models (peak +327 % on Qwen3-4B-Thinking Q8_0). For the current state of the RTX 4050 optimization ceiling and the rolling comparison vs llama.cpp, see `docs/optimization/llamacpp-comparison.md`, `BENCHMARKS.md` (Results v1.13.0 and CPU-only sweep v1.12.0-dev), and the optimization journal under `docs/optimization/`.
