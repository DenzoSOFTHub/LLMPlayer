/**
 * Q5_0 dequantize + matrix-vector multiply kernel.
 * 32 weights per block, 22 bytes per block.
 * Layout: [scale:fp16 (2B)][uint32 qh (4B)][16 bytes nibbles (32 x 4-bit + 1 high bit)]
 * Element mapping (SPLIT, not interleaved):
 *   Elements  0..15 = LOW nibbles of bytes 0..15, high bits from qh bits 0..15
 *   Elements 16..31 = HIGH nibbles of bytes 0..15, high bits from qh bits 16..31
 * value = ((nibble | (high_bit << 4)) - 16) * scale
 *
 * NOTE: Q5_0 block size is 22 bytes — NOT divisible by 4.
 * Block offsets are NOT guaranteed to be 4-byte aligned.
 * All multi-byte reads use byte-level assembly to avoid alignment faults.
 */
__device__ __forceinline__ float half2float(unsigned short h) {
    unsigned int sign = (h >> 15) & 1;
    unsigned int exp = (h >> 10) & 0x1F;
    unsigned int mantissa = h & 0x3FF;
    if (exp == 0) {
        if (mantissa == 0) return sign ? -0.0f : 0.0f;
        while (!(mantissa & 0x400)) { mantissa <<= 1; exp--; }
        exp++; mantissa &= 0x3FF;
    } else if (exp == 31) {
        unsigned int f = (sign << 31) | 0x7F800000 | (mantissa << 13);
        return *(float*)&f;
    }
    unsigned int f = (sign << 31) | ((exp + 112) << 23) | (mantissa << 13);
    return *(float*)&f;
}

extern "C" __global__ void matmul_q5_0(
    const unsigned char* __restrict__ weights,
    const float* __restrict__ input,
    float* __restrict__ output,
    const int rows,
    const int cols,
    const int addToOutput)
{
    int warpId = threadIdx.x / 32;
    int lane = threadIdx.x & 31;
    int rowsPerBlock = blockDim.x / 32;
    int row = blockIdx.x * rowsPerBlock + warpId;
    if (row >= rows) return;

    int numBlocks = cols / 32;
    int rowStride = numBlocks * 22;
    float sum = 0.0f;

    for (int b = lane; b < numBlocks; b += 32) {
        int bo = row * rowStride + b * 22;

        // Read fp16 scale byte-by-byte (block offset not guaranteed 2-byte aligned)
        unsigned short scaleBits = (unsigned short)__ldg(weights + bo)
                                 | ((unsigned short)__ldg(weights + bo + 1) << 8);
        float scale = half2float(scaleBits);

        // Read qh (uint32) byte-by-byte (not guaranteed 4-byte aligned)
        unsigned int qh = (unsigned int)__ldg(weights + bo + 2)
                        | ((unsigned int)__ldg(weights + bo + 3) << 8)
                        | ((unsigned int)__ldg(weights + bo + 4) << 16)
                        | ((unsigned int)__ldg(weights + bo + 5) << 24);

        int inputBase = b * 32;

        // Process 16 packed bytes -> 32 elements (split layout)
        #pragma unroll
        for (int j = 0; j < 16; j++) {
            unsigned char packed = __ldg(weights + bo + 6 + j);
            int lo4 = packed & 0x0F;
            int hi4 = (packed >> 4) & 0x0F;

            int q0 = lo4 | (((qh >> j) & 1) << 4);
            int q1 = hi4 | (((qh >> (j + 16)) & 1) << 4);

            sum += scale * (float)(q0 - 16) * input[inputBase + j];
            sum += scale * (float)(q1 - 16) * input[inputBase + 16 + j];
        }
    }

    // Warp shuffle reduction
    for (int offset = 16; offset > 0; offset >>= 1)
        sum += __shfl_down_sync(0xFFFFFFFF, sum, offset);

    if (lane == 0) {
        if (addToOutput) output[row] += sum;
        else output[row] = sum;
    }
}
