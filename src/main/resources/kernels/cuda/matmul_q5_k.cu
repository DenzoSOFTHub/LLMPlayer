 ...
 * 256 weights per super-block, 176 bytes per block, 4 groups of 64 elements per block.
 * Layout: [d:fp16][dmin:fp16][scales:12B][qh:32B][qs:128B]
 *
 * Threads stripe across GROUPS for full warp utilization.
...
extern "C" __global__ void matmul_q5_k(
    ...
    const int addToOutput)
{
    ...
    for (int g = lane; g < numGroups; g += 32) {
        ...
        if (sb0 < 4) {
            ...
            scale0 = (__ldg(&weights[bo + 4 + sb0 + 4]) & 0x0F)
                   | ((__ldg(&weights[bo + 4 + sb0 - 4]) >> 6) << 4);
            min0 = ((__ldg(&weights[bo + 4 + sb0 + 4]) >> 4) & 0x0F)
                 | ((__ldg(&weights[bo + 4 + sb0]) >> 6) << 4);
        }
        if (sb1 < 4) {
            scale1 = __ldg(&weights[bo + 4 + sb1]) & 0x3F;
            min1 = __ldg(&weights[bo + 4 + sb1 + 4]) & 0x3F;
        } else {
            scale1 = (__ldg(&weights[bo + 4 + sb1 + 4]) & 0x0F)
                   | ((__ldg(&weights[bo + 4 + sb1 - 4]) >> 6) << 4);
            min1 = ((__ldg(&weights[bo + 4 + sb1 + 4]) >> 4) & 0x0F)
                 | ((__ldg(&weights[bo + 4 + sb1]) >> 6) << 4);
        }
        ...
        for (int l = 0; l < 32; l++) {
            ...
            sum += w0 * __ldg(&input[inputBase + l]);
            sum += w1 * __ldg(&input[inputBase + 32 + l]);
        }
    }
