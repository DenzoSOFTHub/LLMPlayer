/**
 * Q4_K multi-block matmul kernel.
 * Multiple blocks process the same row for better SM occupancy.
 * Each block handles a stripe of Q4_K blocks, partial results via atomicAdd.
 *
 * Enabled via -Dcuda.q4k.multiblock=true
 *
 * Grid: (rows, numStripes) — 2D grid, each row × stripe pair processes a slice
 * Block: 32 threads (1 warp)
 */
__device__ __forceinline__ float half2float_mb(unsigned short h) {
...
extern "C" __global__ void matmul_q4_k_multiblock(
    ...
    const int numStripes)
{
    ...
    // Each stripe handles a portion of groups
    int groupsPerStripe = (numGroups + numStripes - 1) / numStripes;
    int groupStart = stripe * groupsPerStripe;
    int groupEnd = groupStart + groupsPerStripe;
    if (groupEnd > numGroups) groupEnd = numGroups;

    long rowOffset = (long)row * numBlocks * 144;
    float sum = 0.0f;

    for (int g = groupStart + lane; g < groupEnd; g += 32) {
        int b = g >> 2;
        int group = g & 3;
        long bo = rowOffset + (long)b * 144;

        unsigned int dm = __ldg((const unsigned int*)(weights + bo));
        float d = half2float_mb(dm & 0xFFFF);
        float dmin = half2float_mb(dm >> 16);
        ...
        if (sb0 < 4) {
            scale0 = sb[sb0] & 0x3F; min0 = sb[sb0 + 4] & 0x3F;
        } else {
            scale0 = (sb[sb0 + 4] & 0x0F) | ((sb[sb0 - 4] >> 6) << 4);
            min0 = ((sb[sb0 + 4] >> 4) & 0x0F) | ((sb[sb0] >> 6) << 4);
        }
        if (sb1 < 4) {
            scale1 = sb[sb1] & 0x3F; min1 = sb[sb1 + 4] & 0x3F;
        } else {
    ...
    if (lane == 0) {
        // All stripes atomicAdd — output must be pre-cleared (or addToOutput=1)
